...
# - **Calibration set** (`X_cal`): calibrates conformal predictors (never seen by the base regressor during training)
# - **Test set** (`X_test`): evaluates coverage and interval width
# 
# We standardize X and y so that difficulty estimates and the SR loss function are scale-invariant.
...
# 50 / 25 / 25 split (matching run_experiments.py)
...
# 
# `crepes.WrapRegressor` wraps any sklearn-compatible regressor and adds conformal prediction methods (`.calibrate()` and `.predict_int()`).
# 
# We use `oob_score=True` so the Random Forest stores *out-of-bag (OOB) predictions* — predictions made on each training sample by trees that did not train on it. These are used later as one of the difficulty estimators.
...
# 
# The idea is that instead of using Symbolic Regression to directly predict confidence bounds, we use it as a difficulty estimator to use in Normalized Conformal Prediction (NCP). Since we need extra data to train the symbolic regressor, we can make use of Random Forest's out-of-bag predictions.
...
# TODO: can we include sigmas?

# %% [markdown]
# ### 5.1. MAE fitness function
...
y_train_sr = np.log(np.abs(residuals_prop_oob))
...
# wrapped learner (needs the raw data) and de.apply()
# (needs the sigma-augmented columns) — incompatible here, so calibrate
# manually via the lower-level ConformalRegressor instead.
...
conf_intervals["symbolic_regression"] = cr_sr.predict_int(
    learner_prop.predict(X_test), sigmas=sigmas_test_sr, confidence=confidence
)
sigmas["symbolic_regression"] = sigmas_cal_sr
print(f"Chosen SR expression: {sigma_predictor.sympy()}")
